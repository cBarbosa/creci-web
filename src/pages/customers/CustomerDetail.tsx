...
const CustomerDetailPage: React.FunctionComponent<ICustomerDetailPageProps> = (props) => {
    ...
    // const [previousPages, setPreviousPages] = React.useState<number[]>([]);
    ...
    //     return [...new Array(to - from)].map((v, index) => {
    //         return from + index + 1;
    //     }).filter(page => page > 0);
    // };
    ...
    //  React.useEffect(() => {
    //     if(currentPage > 1) {
    //         setPreviousPages(generatePagesArray(currentPage - 1 - siblingsCount, currentPage - 1));
    //     } else {
    //         setPreviousPages([]);
    //     }
    //  }, [currentPage]);
